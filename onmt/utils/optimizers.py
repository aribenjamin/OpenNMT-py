...
def build_torch_optimizer(model, opt):
    ...
    if opt.optim == 'sgd':
        ...
        optimizer = optim.Adam(
            ...
            betas=betas,
            eps=1e-9)
    elif opt.optim == 'hcadam':
        optimizer = HCAdam(params,
                        lr=opt.learning_rate,
                        fcn_change_limiter=opt.hcadam_fcl,
                        inner_lr=opt.learning_rate*10,
                        n_corrections = opt.hcadam_ncorrs,  # HC params
                        betas=betas,
                        eps=1e-9)
...
class Optimizer(object):
    ...
    def __init__(self,
       ...
       Args:
         ...
         hilbert_corrected = True if using hcadam optimzer
        """
    ...
    def step(self, val_eval = None, orig_val_output = None):
        ...
        if self.hilbert_corrected:
            self._optimizer.step(val_eval, orig_val_output = orig_val_output)
        else:
...
class HCAdam(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr=None, fcn_change_limiter=0.5, inner_lr=0.01,
    ...
    def step(self, validation_eval, closure=None, orig_val_output=None):
        ...
        # distance metric being L2 distance)
        for i in range(self.n_corrections):
            ...
            for group, group_jacobian in zip(self.param_groups, jacobian):
                ...
                for p, j in zip(group['params'], group_jacobian):
                    ...
                    amsgrad = group['amsgrad']
                    L2_grad = p.grad.data
                    scaled_L2_grad = L2_grad.mul(fcn_change_limiter)
